...
def _layers_list(result: ModelResult, exclude_basement: bool) -> list[str]:
    ...
    for stat in result.lithology_stats:
        if stat.id not in seen:
            if exclude_basement and stat.element_name.lower() == "basement":
                continue
            seen.add(stat.id)
...
def _filter_basement(boundaries: list[dict], exclude_basement: bool) -> list[dict]:
    ...
    return [
        ...
        if b.get("upper_element", "").lower() != "basement"
        and b.get("lower_element", "").lower() != "basement"
    ]
...
def to_section_json(
    ...
    for (upper_id, lower_id), points in boundary_points.items():
        ...
        if exclude_basement and (
            lower_name.lower() == "basement" or upper_name.lower() == "basement"
        ):
...
def to_path_section_json(
    ...
    if has_skimage:
        try:
            ...
            logger.warning("Contour extraction failed, using discrete: %s", e)
